...
public partial class CheckoutController : BasePublicController
{
    ...
    protected readonly ICheckoutDtoFactory _checkoutModelFactory;
    ...
    public CheckoutController(AddressSettings addressSettings,
        ...
        ICheckoutDtoFactory checkoutModelFactory,
    ...
    protected virtual async Task<JsonResult> EditAddressAsync(AddressDto addressModel, IFormCollection form, Func<Customer, IList<ShoppingCartItem>, Address, Task<JsonResult>> getResult)
    {
        try
        {
            ...
            if (customAttributeWarnings.Any())
                return new JsonResult (new { error = 1, message = customAttributeWarnings });
        ...
        catch (Exception exc)
        {
            ...
            return new JsonResult (new { error = 1, message = exc.Message });
        }
    }
    ...
    [ProducesResponseType(typeof(CheckoutBillingAddressDto), (int)HttpStatusCode.OK)]
    ...
    public virtual async Task<IActionResult> Index()
    {
        ...
        if (_orderSettings.CheckoutDisabled)
            return Error(errorMessage: "Disabled form settings");// RedirectToRoute("ShoppingCart");
        ...
        if (!nonButtonPaymentMethods.Any() && buttonPaymentMethods.Any())
            return Error(); // RedirectToRoute("ShoppingCart");
    ...
    [ProducesResponseType(typeof(CompletedResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> Completed([FromQuery] int? orderId)
    {
        ...
        {
            return Error(errorMessage:"order is null, deleted, or dosent belong to current custumer");
        }
        ...
        //model
        var model = await _checkoutModelFactory.PrepareCheckoutCompletedDtoAsync(order);
        return Ok(model);
    }
    ...
    [HttpGet]
    [Route("GetAddressById/{addressId}", Name = "GetAddressById")]
    [ProducesResponseType(typeof(GetAddressByIdResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> GetAddressById([FromRoute] int addressId)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var address = await _customerService.GetCustomerAddressAsync(customer.Id, addressId);
        ...
        await _addressModelFactory.PrepareAddressDtoAsync(addressModel,
            address: address,
    ...
    /// <returns></returns>
    [HttpPost]
    [Route("SaveEditAddress", Name = "SaveEditAddress")]
    [ProducesResponseType(typeof(CheckoutBillingAddressDtoEditAddressResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SaveEditBillingAddress(CheckoutBillingAddressDto model, IFormCollection form, [FromQuery] bool opc = false)
    {
        return await EditAddressAsync(model.BillingNewAddress, form, async (customer, cart, address) =>
        {
            ...
            if (!opc)
                return new JsonResult (new { redirect = Url.RouteUrl("CheckoutBillingAddress") });

            var billingAddressDto =
                await _checkoutModelFactory.PrepareBillingAddressDtoAsync(cart, address.CountryId);

            return new JsonResult (new
            {
                selected_id = model.BillingNewAddress.Id,
                update_section = billingAddressDto
            });
        });
    }
    ...
    [HttpDelete]
    [Route("DeleteEditAddress/{addressId}", Name = "DeleteEditAddress")]
    [ProducesResponseType(typeof(CheckoutBillingAddressDtoEditAddressResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    public virtual async Task<IActionResult> DeleteEditBillingAddress([FromRoute] int addressId, [FromQuery] bool opc = false)
    {
        return await DeleteAddressAsync(addressId, async (cart) =>
        {
            ...
            var billingAddressDto = await _checkoutModelFactory.PrepareBillingAddressDtoAsync(cart);
            return new JsonResult (new
            {
                update_section = new 
                {
    ...
    /// <param name="opc"></param>
    [HttpDelete]
    [Route("DeleteEditShippingAddress/{addressId}", Name = "DeleteEditShippingAddress")]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    public virtual async Task<IActionResult> DeleteEditShippingAddress(int addressId, bool opc = false)
    {
        ...
        {
            if (!opc)
                return new JsonResult (new { redirect = Url.RouteUrl("CheckoutShippingAddress") });

            var shippingAddressDto = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart);
    ...
    /// <returns></returns>
    [HttpPost]
    [Route("SaveEditShippingAddress", Name = "SaveEditShippingAddress")]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SaveEditShippingAddress(CheckoutShippingAddressDto model, IFormCollection form, bool opc = false)
    {
        return await EditAddressAsync(model.ShippingNewAddress, form, async (customer, cart, address) =>
        {
            ...
            var shippingAddressDto = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart, address.CountryId);
            return new JsonResult (new
            {
    ...
    [HttpPost]
    [Route("BillingAddress", Name = "BillingAddress")]
    [ProducesResponseType(typeof(CheckoutBillingAddressDto), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> BillingAddress(IFormCollection form)
    {
        ...
        //model
        var model = await _checkoutModelFactory.PrepareBillingAddressDtoAsync(cart, prePopulateNewAddressWithCustomerFields: true);
    ...
    [HttpGet]
    [Route("SelectBillingAddress/{addressId}", Name = "SelectBillingAddress")]
    [ProducesResponseType(typeof(CheckoutRedirectResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SelectBillingAddress([FromRoute] int addressId, [FromQuery] bool shipToSameAddress = false)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return RedirectToRoute("ShoppingCart");
    ...
    [HttpPost]
    [Route("NewBillingAddress", Name = "NewBillingAddress")]
    [ProducesResponseType(typeof(NewBillingAddressResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> NewBillingAddress(CheckoutBillingAddressDto model, IFormCollection form)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return Error();// RedirectToRoute("ShoppingCart");
        ...
        if (_orderSettings.OnePageCheckoutEnabled)
            return Error(); // RedirectToRoute("CheckoutOnePage");
        ...
        model = await _checkoutModelFactory.PrepareBillingAddressDtoAsync(cart,
    ...
    [HttpGet]
    [Route("ShippingAddress", Name = "ShippingAddress")]
    ...
    public virtual async Task<IActionResult> ShippingAddress()
    {
        ...
        //model
        var model = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart, prePopulateNewAddressWithCustomerFields: true);
        return Ok(model);
    }

    [HttpGet]
    [Route("SelectShippingAddress/{addressId}", Name = "SelectShippingAddress")]
    [ProducesResponseType(typeof(CheckoutRedirectResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SelectShippingAddress([FromRoute] int addressId)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return Error(); // RedirectToRoute("ShoppingCart");
        ...
        if (address == null)
            return NotFound(); // RedirectToRoute("CheckoutShippingAddress");
    ...
    [FormValueRequired("nextstep")]
    [HttpPost]
    [Route("NewShippingAddress", Name = "NewShippingAddress")]
    [ProducesResponseType(typeof(NewShippingAddressResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> NewShippingAddress(CheckoutShippingAddressDto model, IFormCollection form)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return Error(); // RedirectToRoute("ShoppingCart");
        ...
        //If we got this far, something failed, redisplay form
        model = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart,
            selectedCountryId: newAddress.CountryId,
            overrideAttributesXml: customAttributes);
        return Ok(model);
    }
    ...
    [HttpGet]
    [Route("ShippingMethod", Name = "ShippingMethod")]
    [ProducesResponseType(typeof(ShippingMethodResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> ShippingMethod()
    {
        ...
        //model
        var model = await _checkoutModelFactory.PrepareShippingMethodDtoAsync(cart, await _customerService.GetCustomerShippingAddressAsync(customer));
    ...
    [HttpPost]
    [Route("SelectShippingMethod", Name = "SelectShippingMethod")]
    [ProducesResponseType(typeof(CheckoutRedirectResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SelectShippingMethod([FromQuery] string shippingoption, IFormCollection form)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return Error(); // RedirectToRoute("ShoppingCart");
    ...
    public virtual async Task<IActionResult> PaymentMethod()
    {
        ...
        //model
        var paymentMethodModel = await _checkoutModelFactory.PreparePaymentMethodDtoAsync(cart, filterByCountryId);
    ...
    [FormValueRequired("nextstep")]
    [HttpPost]
    [Route("SelectPaymentMethod", Name = "SelectPaymentMethod")]
    [ProducesResponseType(typeof(CheckoutRedirectResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> SelectPaymentMethod([FromQuery] string paymentmethod, CheckoutPaymentMethodDto model)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return Error(); // RedirectToRoute("ShoppingCart");
    ...
    [HttpGet]
    [Route("PaymentInfo", Name = "PaymentInfo")]
    ...
    [HttpPost]
    [Route("EnterPaymentInfo", Name = "EnterPaymentInfo")]
    [ProducesResponseType(typeof(CheckoutConfirmDto), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    ...
    public virtual async Task<IActionResult> Confirm()
    {
        ...
        //model
        var model = await _checkoutModelFactory.PrepareConfirmOrderDtoAsync(cart);
        return Ok(model);
    }

    [HttpGet]
    [Route("ConfirmOrder", Name = "ConfirmOrder")]
    [ProducesResponseType(typeof(ConfirmOrderResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    ...
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var shippingMethodModel = await _checkoutModelFactory.PrepareShippingMethodDtoAsync(cart, await _customerService.GetCustomerShippingAddressAsync(customer));
        if (_shippingSettings.BypassShippingMethodSelectionIfOnlyOne &&
    ...
    protected virtual async Task<JsonResult> OpcLoadStepAfterShippingMethod(IList<ShoppingCartItem> cart)
    {
        ...
        var confirmOrderDto = await _checkoutModelFactory.PrepareConfirmOrderDtoAsync(cart);
        return new JsonResult (new
        {
            update_section = new 
            {
                name = "confirm-order",
                model = confirmOrderDto
            },
    ...
    [HttpGet]
    [Route("OnePageCheckout", Name = "OnePageCheckout")]
    [ProducesResponseType(typeof(OnePageCheckoutDto), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OnePageCheckout()
    {
        ...
        var model = await _checkoutModelFactory.PrepareOnePageCheckoutDtoAsync(cart);
        return Ok(model);
    }


    [HttpPost]
    [Route("OpcSaveBilling", Name = "OpcSaveBilling")]
    [ProducesResponseType(typeof(CheckoutBillingAddressDtoNextStepResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcSaveBilling(CheckoutBillingAddressDto model, IFormCollection form)
    {
        try
        {
            //validation
            ...
            if (!_orderSettings.OnePageCheckoutEnabled)
                return Error(); // throw new Exception("One page checkout is disabled");

            if (await _customerService.IsGuestAsync(customer) && !_orderSettings.AnonymousCheckoutAllowed)
                return Error(); // throw new Exception("Anonymous checkout is not allowed");
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    var billingAddressDto = await _checkoutModelFactory.PrepareBillingAddressDtoAsync(cart,
                        selectedCountryId: newAddress.CountryId,
                        overrideAttributesXml: customAttributes);
                    billingAddressDto.NewAddressPreselected = true;
            ...
            if (await _shoppingCartService.ShoppingCartRequiresShippingAsync(cart))
            {
                ...
                var shippingAddressDto = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart, prePopulateNewAddressWithCustomerFields: true);
        ...
        {
            await _logger.WarningAsync(exc.Message, exc, await _workContext.GetCurrentCustomerAsync());
            return Ok(new { error = 1, message = exc.Message });
        }
    }


    [HttpPost]
    [Route("OpcSaveShipping", Name = "OpcSaveShipping")]
    [ProducesResponseType(typeof(CheckoutShippingAddressDtoNextStepResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcSaveShipping(CheckoutShippingAddressDto model, IFormCollection form)
    {
        try
        {
            //validation
            ...
            if (!await _shoppingCartService.ShoppingCartRequiresShippingAsync(cart))
                return Error(); // throw new Exception("Shipping is not required");
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    var shippingAddressDto = await _checkoutModelFactory.PrepareShippingAddressDtoAsync(cart,
                        selectedCountryId: newAddress.CountryId,
                        overrideAttributesXml: customAttributes);
                    shippingAddressDto.NewAddressPreselected = true;
                    return new JsonResult (new
                    {
                        update_section = new 
                        {
    ...
    [HttpPost]
    [Route("OpcSaveShippingMethod", Name = "OpcSaveShippingMethod")]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcSaveShippingMethod([FromQuery] string shippingoption, IFormCollection form)
    {
        try
        {
            //validation
    ...
    [HttpPost]
    [Route("OpcSavePaymentMethod", Name = "OpcSavePaymentMethod")]
    [ProducesResponseType(typeof(CheckoutConfirmDtoNextStepResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcSavePaymentMethod([FromQuery] string paymentmethod, CheckoutPaymentMethodDto model)
    {
        try
        {
            //validation
            ...
            //payment method 
            if (string.IsNullOrEmpty(paymentmethod))
                return Error(); // throw new Exception("Selected payment method can't be parsed");
    ...
    [HttpPost]
    [Route("OpcSavePaymentInfo", Name = "OpcSavePaymentInfo")]
    [ProducesResponseType(typeof(CheckoutConfirmDtoNextStepResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    ...
    [HttpGet]
    [Route("OpcConfirmOrder", Name = "OpcConfirmOrder")]
    [ProducesResponseType(typeof(CheckoutConfirmDtoNextStepResponse), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Found)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcConfirmOrder(bool captchaValid)
    {
        try
        {
            ...
            var confirmOrderDto = new CheckoutConfirmDto()
            {
            ...
            if (!isCaptchaSettingEnabled || (isCaptchaSettingEnabled && captchaValid))
            {
                ...
                //prevent 2 orders being placed within an X seconds time frame
                if (!await IsMinimumOrderPlacementIntervalValidAsync(customer))
                    return Error(); // throw new Exception(await _localizationService.GetResourceAsync("Checkout.MinOrderPlacementInterval"));
                ...
                //error
                foreach (var error in placeOrderResult.Errors)
                    confirmOrderDto.Warnings.Add(error);
            }
            else
            {
                confirmOrderDto.Warnings.Add(await _localizationService.GetResourceAsync("Common.WrongCaptchaMessage"));
            }
    ...
    [HttpPost]
    [Route("OpcCompleteRedirectionPayment", Name = "OpcCompleteRedirectionPayment")]
    [ProducesResponseType(typeof(CheckoutCompletedDto), (int)HttpStatusCode.OK)]
    [ProducesResponseType(typeof(ProblemDetails), (int)HttpStatusCode.BadRequest)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
    [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
    public virtual async Task<IActionResult> OpcCompleteRedirectionPayment()
    {
        try
        {
            ...
            if (paymentMethod == null)
                return NotFound();
            if (paymentMethod.PaymentMethodType != PaymentMethodType.Redirection)
                return Error(errorMessage:"payment method is not redirction");

            //ensure that order has been just placed
            if ((DateTime.UtcNow - order.CreatedOnUtc).TotalMinutes > 3)
                return Error(errorMessage: "ensure that order has been just placed");
    ...
    #region Components
    [HttpGet]
    [Route("GetCheckoutProgress", Name = "GetCheckoutProgress")]
    [ProducesResponseType(typeof(CheckoutProgressDto), (int)HttpStatusCode.OK)]
    public async Task<IActionResult> GetCheckoutProgress(CheckoutProgressStep step)
    {
        var model = await _checkoutModelFactory.PrepareCheckoutProgressDtoAsync(step);
        return Ok(model);
    }
    #endregion
}
