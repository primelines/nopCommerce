...
/// </summary>
public partial class CustomerDtoFactory : ICustomerDtoFactory
{
    #region Fields
    ...
    public CustomerDtoFactory(AddressSettings addressSettings,
    ...
    protected virtual async Task<GdprConsentDto> PrepareGdprConsentDtoAsync(GdprConsent consent, bool accepted)
    {
        ArgumentNullException.ThrowIfNull(consent);

        var requiredMessage = await _localizationService.GetLocalizedAsync(consent, x => x.RequiredMessage);
        return new GdprConsentDto
        {
            Id = consent.Id,
    ...
    /// </returns>
    public virtual async Task<CustomerInfoDto> PrepareCustomerInfoDtoAsync(CustomerInfoDto model, Customer customer,
        bool excludeProperties, string overrideCustomCustomerAttributesXml = "")
    {
        ...
        foreach (var tzi in _dateTimeHelper.GetSystemTimeZones())
            model.AvailableTimeZones.Add(new SelectListItemDto { Text = tzi.DisplayName, Value = tzi.Id, Selected = (excludeProperties ? tzi.Id == model.TimeZoneId : tzi.Id == (await _dateTimeHelper.GetCurrentTimeZoneAsync()).Id) });
        ...
        if (!excludeProperties)
        {
            ...
            model.Newsletter = newsletters.Any(x=> x.Active) ;
        ...
        if (_customerSettings.CountryEnabled)
        {
            ...
            model.AvailableCountries.Add(new SelectListItemDto { Text = await _localizationService.GetResourceAsync("Address.SelectCountry"), Value = "0" });
            ...
            if (_customerSettings.StateProvinceEnabled)
            {
                ...
                if (states.Any())
                {
                    model.AvailableStates.Add(new SelectListItemDto { Text = await _localizationService.GetResourceAsync("Address.SelectState"), Value = "0" });

                    foreach (var s in states)
                    {
                        model.AvailableStates.Add(new SelectListItemDto { Text = await _localizationService.GetLocalizedAsync(s, x => x.Name), Value = s.Id.ToString(), Selected = (s.Id == model.StateProvinceId) });
                    }
                }
                else
                {
        ...
        foreach (var record in await _externalAuthenticationService.GetCustomerExternalAuthenticationRecordsAsync(customer))
        {
            ...
            model.AssociatedExternalAuthRecords.Add(new CustomerInfoDto.AssociatedExternalAuthModel
            {
                Id = record.Id,
                Email = record.Email,
        ...
        if (_gdprSettings.GdprEnabled)
        {
            ...
            foreach (var consent in consents)
            {
                ...
                model.GdprConsents.Add(await PrepareGdprConsentDtoAsync(consent, accepted.HasValue && accepted.Value));
            }
        }
    ...
    /// </returns>
    public virtual async Task<RegisterDto> PrepareRegisterDtoAsync(RegisterDto model, bool excludeProperties,
        string overrideCustomCustomerAttributesXml = "", bool setDefaultValues = false)
    {
        ...
        if (setDefaultValues)
        {
            ...
            model.Newsletter = _customerSettings.NewsletterEnabled;
        }
        ...
        if (_gdprSettings.GdprEnabled)
        {
            ...
            foreach (var consent in consents)
            {
                model.GdprConsents.Add(await PrepareGdprConsentDtoAsync(consent, false));
            }
        }
    ...
    /// </returns>
    public virtual Task<LoginDto> PrepareLoginDtoAsync(bool? checkoutAsGuest)
    {
    ...
    /// </returns>
    public virtual Task<PasswordRecoveryDto> PreparePasswordRecoveryModelAsync(PasswordRecoveryDto model)
    {
        ArgumentNullException.ThrowIfNull(model);
    ...
    /// </returns>
    public virtual async Task<RegisterResultDto> PrepareRegisterResultDtoAsync(int resultId, string returnUrl)
    {
        ...
        var model = new RegisterResultDto
        {
            Result = resultText,
            ReturnUrl = returnUrl
        };
    ...
    /// </returns>
    public virtual async Task<CustomerNavigationDto> PrepareCustomerNavigationDtoAsync(int selectedTabId = 0)
    {
        var model = new CustomerNavigationDto();
    ...
    /// </returns>
    public virtual async Task<CustomerAddressListDto> PrepareCustomerAddressListDtoAsync()
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        ...
        var model = new CustomerAddressListDto();
    ...
    /// </returns>
    public virtual async Task<CustomerDownloadableProductsDto> PrepareCustomerDownloadableProductsDtoAsync()
    {
        var model = new CustomerDownloadableProductsDto();
        var customer = await _workContext.GetCurrentCustomerAsync();
        ...
        foreach (var item in items)
        {
            ...
            var itemModel = new CustomerDownloadableProductsDto.DownloadableProductsModel
            {
    ...
    /// </returns>
    public virtual Task<UserAgreementDto> PrepareUserAgreementModelAsync(OrderItem orderItem, Product product)
    {
        ArgumentNullException.ThrowIfNull(orderItem);

        ArgumentNullException.ThrowIfNull(product);

        var model = new UserAgreementDto
        {
    ...
    /// </returns>
    public virtual Task<ChangePasswordDto> PrepareChangePasswordDtoAsync()
    {
    ...
    /// </returns>
    public virtual async Task<CustomerAvatarDto> PrepareCustomerAvatarDtoAsync(CustomerAvatarDto model)
    {
        ArgumentNullException.ThrowIfNull(model);
    ...
    /// </returns>
    public virtual Task<GdprToolsDto> PrepareGdprToolsDtoAsync()
    {
        var model = new GdprToolsDto();
    ...
    /// </returns>
    public virtual Task<CheckGiftCardBalanceDto> PrepareCheckGiftCardBalanceDtoAsync()
    {
        var model = new CheckGiftCardBalanceDto();
    ...
    /// </returns>
    public virtual async Task<MultiFactorAuthenticationDto> PrepareMultiFactorAuthenticationDtoAsync(MultiFactorAuthenticationDto model)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        ...
        {
            var providerModel = new MultiFactorAuthenticationProviderDto();
            var sysName = multiFactorAuthenticationProvider.PluginDescriptor.SystemName;
            providerModel = await PrepareMultiFactorAuthenticationProviderDtoAsync(providerModel, sysName);
    ...
    /// The task result contains the multi-factor authentication model
    /// </returns>
    public virtual async Task<MultiFactorAuthenticationProviderDto> PrepareMultiFactorAuthenticationProviderDtoAsync(MultiFactorAuthenticationProviderDto providerModel, string sysName, bool isLogin = false)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
    ...
    /// </returns>
    public virtual async Task<IList<CustomerAttributeDto>> PrepareCustomCustomerAttributesAsync(Customer customer, string overrideAttributesXml = "")
    {
        ArgumentNullException.ThrowIfNull(customer);

        var result = new List<CustomerAttributeDto>();

        var customerAttributes = await _customerAttributeService.GetAllAttributesAsync();
        foreach (var attribute in customerAttributes)
        {
            var attributeModel = new CustomerAttributeDto
            {
                Id = attribute.Id,
            ...
            if (attribute.ShouldHaveValues)
            {
                ...
                {
                    var valueModel = new CustomerAttributeValueDto
                    {
