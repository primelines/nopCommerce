...
using Nop.Api.Framework.Extensions;
...
/// </summary>
public partial class ForumDtoFactory : IForumDtoFactory
{
    #region Fields
    ...
    public ForumDtoFactory(CaptchaSettings captchaSettings,
    ...
    /// </returns>
    protected virtual async Task<IEnumerable<SelectListItemDto>> ForumTopicTypesListAsync()
    {
    ...
    /// The task result contains the collection of the select list item
    /// </returns>
    protected virtual async Task<IEnumerable<SelectListItemDto>> ForumGroupsForumsListAsync()
    {
        var forumsList = new List<SelectListItemDto>();
        var separator = "--";
        ...
        foreach (var fg in forumGroups)
        {
            ...
            forumsList.Add(new SelectListItemDto { Text = fg.Name, Value = "0" });

            var forums = await _forumService.GetAllForumsByGroupIdAsync(fg.Id);
            foreach (var f in forums)
            {
                forumsList.Add(new SelectListItemDto { Text = $"{separator}{f.Name}", Value = f.Id.ToString() });
            }
        }
    ...
    /// </returns>
    public virtual async Task<ForumGroupDto> PrepareForumGroupDtoAsync(ForumGroup forumGroup)
    {
        ArgumentNullException.ThrowIfNull(forumGroup);

        var forumGroupModel = new ForumGroupDto
        {
            Id = forumGroup.Id,
        ...
        {
            var forumModel = await PrepareForumRowDtoAsync(forum);
    ...
    /// </returns>
    public virtual async Task<BoardsIndexDto> PrepareBoardsIndexDtoAsync()
    {
        var model = new BoardsIndexDto();

        var forumGroups = await _forumService.GetAllForumGroupsAsync();
        foreach (var forumGroup in forumGroups)
        {
            var forumGroupModel = await PrepareForumGroupDtoAsync(forumGroup);
    ...
    /// </returns>
    public virtual async Task<ActiveDiscussionsDto> PrepareActiveDiscussionsDtoAsync()
    {
        var model = new ActiveDiscussionsDto
        {
        ...
        {
            var topicModel = await PrepareForumTopicRowDtoAsync(topic);
    ...
    /// The task result contains the active discussions model
    /// </returns>
    public virtual async Task<ActiveDiscussionsDto> PrepareActiveDiscussionsDtoAsync(int forumId, int page)
    {
        var model = new ActiveDiscussionsDto
        {
    ...
    /// </returns>
    public virtual async Task<ForumPageDto> PrepareForumPageDtoAsync(Forum forum, int page)
    {
        ArgumentNullException.ThrowIfNull(forum);

        var model = new ForumPageDto
        {
            Id = forum.Id,
            Name = forum.Name,
    ...
    /// </returns>
    public virtual async Task<ForumTopicPageDto> PrepareForumTopicPageDtoAsync(ForumTopic forumTopic, int page)
    {
        ...
        var model = new ForumTopicPageDto
        {
            Id = forumTopic.Id,
        ...
        foreach (var post in posts)
        {
            ...
            var forumPostModel = new ForumPostDto
            {
                Id = post.Id,
            ...
            model.ForumPosts.Add(forumPostModel);
        }
    ...
    /// </returns>
    public virtual async Task<TopicMoveDto> PrepareTopicMoveAsync(ForumTopic forumTopic)
    {
        ArgumentNullException.ThrowIfNull(forumTopic);

        var model = new TopicMoveDto
        {
    ...
    public virtual async Task PrepareTopicCreateModelAsync(Forum forum, EditForumTopicDto model)
    {
        ArgumentNullException.ThrowIfNull(forum);
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareTopicEditModelAsync(ForumTopic forumTopic, EditForumTopicDto model, bool excludeProperties)
    {
        ArgumentNullException.ThrowIfNull(forumTopic);
    ...
    /// </returns>
    public virtual async Task<EditForumPostDto> PreparePostCreateModelAsync(ForumTopic forumTopic, int? quote, bool excludeProperties)
    {
        ArgumentNullException.ThrowIfNull(forumTopic);
        ...
        var currentCustomer = await _workContext.GetCurrentCustomerAsync();
        var model = new EditForumPostDto
        {
    ...
    /// The task result contains the edit forum post model
    /// </returns>
    public virtual async Task<EditForumPostDto> PreparePostEditModelAsync(ForumPost forumPost, bool excludeProperties)
    {
    ...
    /// </returns>
    public virtual async Task<BoardsSearchDto> PrepareSearchDtoAsync(string searchterms, bool? advs, string forumId,
        string within, string limitDays, int page)
    {
        var model = new BoardsSearchDto();

        var pageSize = 10;

        // Create the values for the "Limit results to previous" select list
        var limitList = new List<SelectListItemDto>
        {
            new() {
        ...
        var forumsSelectList = new List<SelectListItemDto>
        {
            new() {
        ...
        foreach (var fg in forumGroups)
        {
            ...
            forumsSelectList.Add(new SelectListItemDto { Text = fg.Name, Value = "-" });
        ...
        var withinList = new List<SelectListItemDto>
        {
            new() {
    ...
    /// </returns>
    public virtual async Task<LastPostDto> PrepareLastPostDtoAsync(ForumPost forumPost, bool showTopic)
    {
        var model = new LastPostDto
        {
            ShowTopic = showTopic
        };
    ...
    /// </returns>
    public virtual async Task<ForumBreadcrumbDto> PrepareForumBreadcrumbDtoAsync(int? forumGroupId, int? forumId, int? forumTopicId)
    {
        var model = new ForumBreadcrumbDto();
    ...
    /// </returns>
    public virtual async Task<CustomerForumSubscriptionsDto> PrepareCustomerForumSubscriptionsDtoAsync(int? page)
    {
        var pageIndex = 0;
        if (page > 0)
        {
        ...
        var model = new CustomerForumSubscriptionsDto();
        ...
        foreach (var forumSubscription in list)
        {
            ...
            model.ForumSubscriptions.Add(new ForumSubscriptionDto
            {
        ...
        model.Pager = new PagerDto(_localizationService)
        {
    ...
    /// </returns>
    public virtual async Task<ForumTopicRowDto> PrepareForumTopicRowDtoAsync(ForumTopic topic)
    {
        ...
        var topicModel = new ForumTopicRowDto
        {
            Id = topic.Id,
    ...
    /// </returns>
    public virtual async Task<ForumRowDto> PrepareForumRowDtoAsync(Forum forum)
    {
        ArgumentNullException.ThrowIfNull(forum);

        var forumModel = new ForumRowDto
        {
            Id = forum.Id,
            Name = forum.Name,
            SeName = await _forumService.GetForumSeNameAsync(forum),
