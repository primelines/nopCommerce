...
/// </summary>
public partial class OrderDtoFactory : IOrderDtoFactory
{
    #region Fields
    ...
    public OrderDtoFactory(AddressSettings addressSettings,
    ...
    /// </returns>
    protected virtual async Task<PictureDto> PrepareOrderItemPictureDtoAsync(OrderItem orderItem, int pictureSize, bool showDefaultPicture, string productName)
    {
        ...
        }, NopDtoCacheDefaults.OrderPictureDtoKey, orderItem, pictureSize, showDefaultPicture, language, _webHelper.IsCurrentConnectionSecured(), store);
    ...
    /// </returns>
    public virtual async Task<CustomerOrderListDto> PrepareCustomerOrderListDtoAsync()
    {
        var model = new CustomerOrderListDto();
        ...
        {
            var orderModel = new CustomerOrderListDto.CustomerOrderDetailsDto
            {
                Id = order.Id,
    ...
    /// </returns>
    public virtual async Task<OrderDetailsDto> PrepareOrderDetailsDtoAsync(Order order)
    {
        ...
        if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
        {
            ...
            {
                var shipmentModel = new ShipmentBriefDto
                {
                    Id = shipment.Id,
        ...
        {
            model.GiftCards.Add(new GiftCardOverViewDto
            {
        ...
        foreach (var orderNote in (await _orderService.GetOrderNotesByOrderIdAsync(order.Id, true))
                 ...
                 .ToList())
        {
            model.OrderNotes.Add(new OrderNoteDto
            {
                Id = orderNote.Id,
        ...
        foreach (var orderItem in orderItems)
        {
            ...
            var orderItemModel = new OrderItemDto
            {
                Id = orderItem.Id,
            ...
            {
                orderItemModel.Picture = await PrepareOrderItemPictureDtoAsync(orderItem, _mediaSettings.OrderThumbPictureSize, true, orderItemModel.ProductName);
            }
        }
    ...
    /// </returns>
    public virtual async Task<ShipmentDetailsDto> PrepareShipmentDetailsDtoAsync(Shipment shipment)
    {
        ArgumentNullException.ThrowIfNull(shipment);

        var order = await _orderService.GetOrderByIdAsync(shipment.OrderId) ?? throw new Exception("order cannot be loaded");
        var model = new ShipmentDetailsDto
        {
            Id = shipment.Id
        };
        if (shipment.ShippedDateUtc.HasValue)
            ...
            if (shipmentTracker != null)
            {
                ...
                if (_shippingSettings.DisplayShipmentEventsToCustomers)
                {
                    ...
                    if (shipmentEvents != null)
                    {
                        ...
                        {
                            var shipmentStatusEventModel = new ShipmentDetailsDto.ShipmentStatusEventModel();
        ...
        foreach (var shipmentItem in await _shipmentService.GetShipmentItemsByShipmentIdAsync(shipment.Id))
        {
            ...
            var shipmentItemModel = new ShipmentDetailsDto.ShipmentItemDto
            {
                Id = shipmentItem.Id,
                Sku = await _productService.FormatSkuAsync(product, orderItem.AttributesXml),
                ProductId = product.Id,
        ...
        //order details model
        model.Order = await PrepareOrderDetailsDtoAsync(order);
    ...
    /// </returns>
    public virtual async Task<CustomerRewardPointsDto> PrepareCustomerRewardPointsAsync(int? page)
    {
        ...
        var model = new CustomerRewardPointsDto
        {
            ...
            Pager = new PagerDto(_localizationService)
            {
