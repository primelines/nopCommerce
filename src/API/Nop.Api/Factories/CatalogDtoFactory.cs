...
public partial class CatalogDtoFactory : ICatalogDtoFactory
{
    #region Fields
    ...
    public CatalogDtoFactory(BlogSettings blogSettings,
    ...
    protected virtual CategorySimpleDto GetCategorySimpleDto(XElement elem)
    {
        var model = new CategorySimpleDto
        {
    ...
    /// </returns>
    protected virtual async Task<PriceRangeDto> GetConvertedPriceRangeAsync(CatalogProductsCommand command)
    {
        var result = new PriceRangeDto();
    ...
    /// </returns>
    protected virtual async Task<SpecificationFilterDto> PrepareSpecificationFilterDto(IList<int> selectedOptions, IList<SpecificationAttributeOption> availableOptions)
    {
        var model = new SpecificationFilterDto();
        ...
        if (availableOptions?.Any() == true)
        {
            ...
            foreach (var option in availableOptions)
            {
                ...
                if (attributeFilter == null)
                {
                    ...
                    attributeFilter = new SpecificationAttributeFilterDto
                    {
                        Id = attribute.Id,
                ...
                attributeFilter.Values.Add(new SpecificationAttributeValueFilterDto
                {
                    Id = option.Id,
                    Name = await _localizationService
    ...
    /// The task result contains the specification filter model
    /// </returns>
    protected virtual async Task<ManufacturerFilterDto> PrepareManufacturerFilterDto(IList<int> selectedManufacturers, IList<Manufacturer> availableManufacturers)
    {
        var model = new ManufacturerFilterDto();
        ...
        if (availableManufacturers?.Any() == true)
        {
            ...
            {
                model.Manufacturers.Add(new SelectListItemDto
                {
    ...
    protected virtual async Task<PriceRangeFilterDto> PreparePriceRangeFilterAsync(PriceRangeDto selectedPriceRange, PriceRangeDto availablePriceRange)
    {
        var model = new PriceRangeFilterDto();
    ...
    protected virtual async Task PrepareCatalogProductsAsync(CatalogProductsDto model, IPagedList<Product> products, bool isFiltering = false)
    {
        ...
        {
            model.Products = (await _productModelFactory.PrepareProductOverviewDtosAsync(products)).ToList();
    ...
    /// </returns>
    public virtual async Task<CategoryDto> PrepareCategoryDtoAsync(Category category, CatalogProductsCommand command)
    {
        ...
        if (_catalogSettings.CategoryBreadcrumbEnabled)
        {
            ...
            {
                var categoryBreadcrumb = model.CategoryBreadcrumb.Select(c => new CategorySimpleDto { Id = c.Id, Name = c.Name, SeName = c.SeName }).ToList();
        ...
        model.SubCategories = await (await _categoryService.GetAllCategoriesByParentCategoryIdAsync(category.Id))
            ...
            {
                var subCatModel = new CategoryDto.SubCategoryDto
                {
                    Id = curCategory.Id,
                ...
                var categoryPictureCacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.CategoryPictureDtoKey, curCategory,
                    pictureSize, true, await _workContext.GetWorkingLanguageAsync(), _webHelper.IsCurrentConnectionSecured(),
                    currentStore);

                subCatModel.Picture = await _staticCacheManager.GetAsync(categoryPictureCacheKey, async () =>
                {
        ...
        if (!_catalogSettings.IgnoreFeaturedProducts)
        {
            ...
            if (featuredProducts != null)
                model.FeaturedProducts = (await _productModelFactory.PrepareProductOverviewDtosAsync(featuredProducts)).ToList();
        }
    ...
    /// </returns>
    public virtual async Task<CategoryNavigationDto> PrepareCategoryNavigationDtoAsync(int currentCategoryId, int currentProductId)
    {
        //get active category
        ...
        var cachedCategoriesModel = await PrepareCategorySimpleDtosAsync();
        var model = new CategoryNavigationDto
        {
    ...
    ///// </returns>
    //public virtual async Task<TopMenuDto> PrepareTopMenuDtoAsync()
    //{
    //    var cachedCategoriesModel = new List<CategorySimpleDto>();
    //    //categories
    //    if (!_catalogSettings.UseAjaxLoadMenu)
    //        cachedCategoriesModel = await PrepareCategorySimpleDtosAsync();

    //    var store = await _storeContext.GetCurrentStoreAsync();

    //    //top menu topics
    //    var topicModel = await (await _topicService.GetAllTopicsAsync(store.Id, onlyIncludedInTopMenu: true))
    //        .SelectAwait(async t => new MenuTopicDto
    //        {
    //            Id = t.Id,
    ...
    /// </returns>
    public virtual async Task<List<CategorySimpleDto>> PrepareRootCategoriesAsync()
    {
        var doc = await PrepareCategoryXmlDocumentAsync();

        var models = from xe in doc.Root.XPathSelectElements("CategorySimpleDto")
            select GetCategorySimpleDto(xe);
    ...
    /// </returns>
    public virtual async Task<List<CategorySimpleDto>> PrepareSubCategoriesAsync(int id)
    {
        var doc = await PrepareCategoryXmlDocumentAsync();

        var model = from xe in doc.Descendants("CategorySimpleDto")
            where xe.XPathSelectElement("Id").Value == id.ToString()
            select xe;

        var models = from xe in model.First().XPathSelectElements("SubCategories/CategorySimpleDto")
            select GetCategorySimpleDto(xe);
    ...
    /// </returns>
    public virtual async Task<CatalogProductsDto> PrepareCategoryProductsModelAsync(Category category, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(category);
        ...
        //price range
        PriceRangeDto selectedPriceRange = null;
        if (_catalogSettings.EnablePriceRangeFiltering && category.PriceRangeFiltering)
        {
            ...
            PriceRangeDto availablePriceRange = null;
        ...
        {
            model.SpecificationFilter = await PrepareSpecificationFilterDto(command.SpecificationOptionIds, filterableOptions);
        }
        ...
        if (_catalogSettings.EnableManufacturerFiltering)
        {
            ...
            model.ManufacturerFilter = await PrepareManufacturerFilterDto(command.ManufacturerIds, manufacturers);
        }
    ...
    /// The task result contains the list of category (simple) models
    /// </returns>
    public virtual async Task<List<CategorySimpleDto>> PrepareCategorySimpleDtosAsync()
    {
        //load and cache them
        var language = await _workContext.GetWorkingLanguageAsync();
        var customer = await _workContext.GetCurrentCustomerAsync();
        var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
        var store = await _storeContext.GetCurrentStoreAsync();
        var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.CategoryAllModelKey,
            language, customerRoleIds, store);

        return await _staticCacheManager.GetAsync(cacheKey, async () => await PrepareCategorySimpleDtosAsync(0));
    }
    ...
    /// The task result contains the list of category (simple) models
    /// </returns>
    public virtual async Task<List<CategorySimpleDto>> PrepareCategorySimpleDtosAsync(int rootCategoryId, bool loadSubCategories = true)
    {
        ...
        foreach (var category in categories)
        {
            ...
            {
                var subCategories = await PrepareCategorySimpleDtosAsync(category.Id);
    ...
    public virtual async Task<XDocument> PrepareCategoryXmlDocumentAsync()
    {
        ...
        var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
        var store = await _storeContext.GetCurrentStoreAsync();
        var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.CategoryXmlAllModelKey,
            language, customerRoleIds, store);

        return await _staticCacheManager.GetAsync(cacheKey, async () =>
        {
            var categories = await PrepareCategorySimpleDtosAsync();

            var xsSubmit = new XmlSerializer(typeof(List<CategorySimpleDto>));
    ...
    /// </returns>
    public virtual async Task<ManufacturerDto> PrepareManufacturerDtoAsync(Manufacturer manufacturer, CatalogProductsCommand command)
    {
    ...
    /// </returns>
    public virtual async Task<CatalogProductsDto> PrepareManufacturerProductsModelAsync(Manufacturer manufacturer, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(manufacturer);
    ...
    /// </returns>
    public virtual async Task<List<ManufacturerDto>> PrepareManufacturerAllModelsAsync()
    {
        var model = new List<ManufacturerDto>();

        var currentStore = await _storeContext.GetCurrentStoreAsync();
        var manufacturers = await _manufacturerService.GetAllManufacturersAsync(storeId: currentStore.Id);
        foreach (var manufacturer in manufacturers)
        {
            var modelMan = new ManufacturerDto
            {
                Id = manufacturer.Id,
                Name = await _localizationService.GetLocalizedAsync(manufacturer, x => x.Name),
                Description = await _localizationService.GetLocalizedAsync(manufacturer, x => x.Description),
            ...
            var manufacturerPictureCacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.ManufacturerPictureDtoKey,
            ...
            modelMan.Picture = await _staticCacheManager.GetAsync(manufacturerPictureCacheKey, async () =>
            {
    ...
    /// </returns>
    public virtual async Task<ManufacturerNavigationDto> PrepareManufacturerNavigationDtoAsync(int currentManufacturerId)
    {
        var language = await _workContext.GetWorkingLanguageAsync();
        var customer = await _workContext.GetCurrentCustomerAsync();
        var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
        var store = await _storeContext.GetCurrentStoreAsync();
        var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.ManufacturerNavigationDtoKey,
        ...
        var cachedModel = await _staticCacheManager.GetAsync(cacheKey, async () =>
        {
            ...
            var model = new ManufacturerNavigationDto
            {
    ...
    /// </returns>
    public virtual async Task<VendorDto> PrepareVendorDtoAsync(Vendor vendor, CatalogProductsCommand command)
    {
        ...
        var model = new VendorDto
        {
            ...
            Bio = await _localizationService.GetLocalizedAsync(vendor, x => x.Description),
            SeName = await _urlRecordService.GetSeNameAsync(vendor),
            AllowCustomersToContactVendors = _vendorSettings.AllowCustomersToContactVendors,
            CatalogProducts = await PrepareVendorProductsModelAsync(vendor, command)
        };
    ...
    /// </returns>
    public virtual async Task<CatalogProductsDto> PrepareVendorProductsModelAsync(Vendor vendor, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(vendor);
    ...
    /// </returns>
    public virtual async Task<List<VendorDto>> PrepareVendorAllModelsAsync()
    {
        var model = new List<VendorDto>();
        var vendors = await _vendorService.GetAllVendorsAsync();
        foreach (var vendor in vendors)
        {
            var vendorModel = new VendorDto
            {
                Id = vendor.Id,
                Name = await _localizationService.GetLocalizedAsync(vendor, x => x.Name),
                Bio = await _localizationService.GetLocalizedAsync(vendor, x => x.Description),
            ...
            var pictureCacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.VendorPictureDtoKey,
            ...
            vendorModel.ProfilePicture = await _staticCacheManager.GetAsync(pictureCacheKey, async () =>
            {
    ...
    /// </returns>
    public virtual async Task<VendorNavigationDto> PrepareVendorNavigationDtoAsync()
    {
        var cacheKey = NopDtoCacheDefaults.VendorNavigationDtoKey;
        var cachedModel = await _staticCacheManager.GetAsync(cacheKey, async () =>
        {
            var vendors = await _vendorService.GetAllVendorsAsync(pageSize: _vendorSettings.VendorsBlockItemsToDisplay);
            var model = new VendorNavigationDto
            {
            ...
            foreach (var vendor in vendors)
            {
                model.Vendors.Add(new VendorBriefInfoDto
                {
                    Id = vendor.Id,
                    Name = await _localizationService.GetLocalizedAsync(vendor, x => x.Name),
                    SeName = await _urlRecordService.GetSeNameAsync(vendor),
    ...
    /// </returns>
    public virtual async Task<PopularProductTagsDto> PreparePopularProductTagsDtoAsync(int numberTagsToReturn = 0)
    {
        var model = new PopularProductTagsDto();
    ...
    /// </returns>
    public virtual async Task<ProductsByTagDto> PrepareProductsByTagDtoAsync(ProductTag productTag, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(productTag);

        ArgumentNullException.ThrowIfNull(command);

        var model = new ProductsByTagDto
        {
            Id = productTag.Id,
            ...
            CatalogProducts = await PrepareTagProductsModelAsync(productTag, command)
        };
    ...
    /// </returns>
    public virtual async Task<CatalogProductsDto> PrepareTagProductsModelAsync(ProductTag productTag, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(productTag);
        ...
        if (_catalogSettings.EnablePriceRangeFiltering && _catalogSettings.ProductsByTagPriceRangeFiltering)
        {
            ...
            PriceRangeDto availablePriceRange;
    ...
    /// </returns>
    public virtual async Task<CatalogSearchDto> PrepareSearchDtoAsync(CatalogSearchDto model, CatalogProductsCommand command)
    {
        ...
        var currentStore = await _storeContext.GetCurrentStoreAsync();
        var categoriesModels = new List<CategoryBreadcrumbDto>();
        //all categories
        ...
        foreach (var c in allCategories)
        {
            ...
            categoriesModels.Add(new CategoryBreadcrumbDto
            {
                Id = c.Id,
        ...
        {
            //first empty entry
            model.AvailableCategories.Add(new SelectListItemDto
            {
                Value = "0",
        ...
        {
            model.AvailableManufacturers.Add(new SelectListItemDto
            {
                Value = "0",
                Text = await _localizationService.GetResourceAsync("Common.All")
            });
        ...
        if (model.asv)
        {
            ...
            if (vendors.Any())
            {
                model.AvailableVendors.Add(new SelectListItemDto
                {
                    Value = "0",
                    Text = await _localizationService.GetResourceAsync("Common.All")
                });
        ...
        model.CatalogProducts = await PrepareSearchProductsModelAsync(model, command);
    ...
    /// </returns>
    public virtual async Task<CatalogProductsDto> PrepareSearchProductsModelAsync(CatalogSearchDto searchModel, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);
    ...
    /// </returns>
    public virtual Task<SearchBoxDto> PrepareSearchBoxDtoAsync()
    {
        var model = new SearchBoxDto
        {
    ...
    /// <param name="command">Model to get the catalog products</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareSortingOptionsAsync(CatalogProductsDto model, CatalogProductsCommand command)
    {
        ...
        {
            model.AvailableSortOptions.Add(new SelectListItemDto
            {
    ...
    /// <param name="command">Model to get the catalog products</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareViewModesAsync(CatalogProductsDto model, CatalogProductsCommand command)
    {
        ...
        {
            //grid
            model.AvailableViewModes.Add(new SelectListItemDto
            {
