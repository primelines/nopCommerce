...
/// </summary>
public partial class NewsDtoFactory : INewsDtoFactory
{
    #region Fields
    ...
    public NewsDtoFactory(CaptchaSettings captchaSettings,
    ...
    /// </returns>
    public virtual async Task<NewsItemDto> PrepareNewsItemDtoAsync(NewsItemDto model, NewsItem newsItem, bool prepareComments)
    {
        ...
        if (prepareComments)
        {
            ...
            {
                var commentModel = await PrepareNewsCommentDtoAsync(nc);
    ...
    /// </returns>
    public virtual async Task<HomepageNewsItemsDto> PrepareHomepageNewsItemsModelAsync()
    {
        var store = await _storeContext.GetCurrentStoreAsync();
        var language = await _workContext.GetWorkingLanguageAsync();
        var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.HomepageNewsModelKey, language, store);
        var cachedModel = await _staticCacheManager.GetAsync(cacheKey, async () =>
        {
            var newsItems = await _newsService.GetAllNewsAsync(language.Id, store.Id, 0, _newsSettings.MainPageNewsCount);

            return new HomepageNewsItemsDto
            {
                WorkingLanguageId = language.Id,
                NewsItems = await newsItems.SelectAwait(async newsItem =>
                {
                    var newsModel = new NewsItemDto();
                    await PrepareNewsItemDtoAsync(newsModel, newsItem, false);
                    return newsModel;
                }).ToListAsync()
            };
        });

        //"Comments" property of "NewsItemDto" object depends on the current customer.
    ...
    /// </returns>
    public virtual async Task<NewsItemListDto> PrepareNewsItemListDtoAsync(NewsPagingFilteringDto command)
    {
    ...
    /// </returns>
    public virtual async Task<NewsCommentDto> PrepareNewsCommentDtoAsync(NewsComment newsComment)
    {
