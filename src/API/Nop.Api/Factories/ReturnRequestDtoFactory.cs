...
/// </summary>
public partial class ReturnRequestDtoFactory : IReturnRequestDtoFactory
{
    #region Fields
    ...
    public ReturnRequestDtoFactory(ICurrencyService currencyService,
    ...
    /// </returns>
    public virtual async Task<SubmitReturnRequestDto> PrepareSubmitReturnRequestDtoAsync(SubmitReturnRequestDto model,
        Order order)
    {
        ...
        model.AvailableReturnReasons = await (await _returnRequestService.GetAllReturnRequestReasonsAsync())
            .SelectAwait(async rrr => new SubmitReturnRequestDto.ReturnRequestReasonDto
            {
                Id = rrr.Id,
        ...
        model.AvailableReturnActions = await (await _returnRequestService.GetAllReturnRequestActionsAsync())
            .SelectAwait(async rra => new SubmitReturnRequestDto.ReturnRequestActionDto
            {
                Id = rra.Id,
        ...
        //returnable products
        model.Items = await PrepareSubmitReturnRequestOrderItemDtosAsync(order);
    ...
    /// </returns>
    public virtual async Task<CustomerReturnRequestsDto> PrepareCustomerReturnRequestsDtoAsync()
    {
        var model = new CustomerReturnRequestsDto();
        ...
        foreach (var returnRequest in returnRequests)
        {
            ...
            if (orderItem != null)
            {
                ...
                var itemModel = new CustomerReturnRequestsDto.ReturnRequestDto
                {
                    Id = returnRequest.Id,
    ...
    /// <param name="order">Order</param>
    /// <returns>
    /// The <see cref="Task"/> containing the <see cref="IList{OrderItemOverviewDto}"/>
    /// </returns>
    protected virtual async Task<IList<OrderItemOverviewDto>> PrepareSubmitReturnRequestOrderItemDtosAsync(Order order)
    {
        ArgumentNullException.ThrowIfNull(order);

        var models = new List<OrderItemOverviewDto>();
        ...
        if (returnRequestAvailability?.IsAllowed == true)
        {
            foreach (var returnableOrderItem in returnRequestAvailability.ReturnableOrderItems)
            {
                ...
                var model = new OrderItemOverviewDto
                {
                    Id = orderItem.Id,
                    ProductId = product.Id,
                    ProductName = await _localizationService.GetLocalizedAsync(product, x => x.Name),
