...
public partial class CheckoutDtoFactory : ICheckoutDtoFactory
{
    #region Fields
    ...
    public CheckoutDtoFactory(AddressSettings addressSettings,
    ...
    /// </returns>
    protected virtual async Task<CheckoutPickupPointsDto> PrepareCheckoutPickupPointsDtoAsync(IList<ShoppingCartItem> cart)
    {
        var model = new CheckoutPickupPointsDto
        {
        ...
        if (pickupPointProviders.Any())
        {
            ...
            if (pickupPointsResponse.Success)
            {
                ...
                model.PickupPoints = await pickupPointsResponse.PickupPoints.SelectAwait(async point =>
                {
                    ...
                    var pickupPointModel = new CheckoutPickupPointDto
                    {
                        Id = point.Id,
                        Name = point.Name,
    ...
    /// </returns>
    public virtual async Task<CheckoutBillingAddressDto> PrepareBillingAddressDtoAsync(IList<ShoppingCartItem> cart,
        int? selectedCountryId = null,
        bool prePopulateNewAddressWithCustomerFields = false,
        string overrideAttributesXml = "")
    {
        var model = new CheckoutBillingAddressDto
        {
        ...
        await _addressModelFactory.PrepareAddressDtoAsync(model.BillingNewAddress,
            address: null,
            excludeProperties: false,
            addressSettings: _addressSettings,
    ...
    /// </returns>
    public virtual async Task<CheckoutShippingAddressDto> PrepareShippingAddressDtoAsync(IList<ShoppingCartItem> cart,
        int? selectedCountryId = null, bool prePopulateNewAddressWithCustomerFields = false, string overrideAttributesXml = "")
    {
        var model = new CheckoutShippingAddressDto
        {
        ...
        if (!_orderSettings.DisplayPickupInStoreOnShippingMethodPage)
            model.PickupPoints = await PrepareCheckoutPickupPointsDtoAsync(cart);
        ...
        await _addressModelFactory.PrepareAddressDtoAsync(model.ShippingNewAddress,
            address: null,
            excludeProperties: false,
            addressSettings: _addressSettings,
    ...
    /// </returns>
    public virtual async Task<CheckoutShippingMethodDto> PrepareShippingMethodDtoAsync(IList<ShoppingCartItem> cart, Address shippingAddress)
    {
    ...
    /// </returns>
    public virtual async Task<CheckoutPaymentMethodDto> PreparePaymentMethodDtoAsync(IList<ShoppingCartItem> cart, int filterByCountryId)
    {
    ...
    /// </returns>
    public virtual Task<CheckoutPaymentInfoDto> PreparePaymentInfoModelAsync(IPaymentMethod paymentMethod)
    {
        return Task.FromResult(new CheckoutPaymentInfoDto
        {
    ...
    /// </returns>
    public virtual async Task<CheckoutConfirmDto> PrepareConfirmOrderDtoAsync(IList<ShoppingCartItem> cart)
    {
        var model = new CheckoutConfirmDto
        {
            //terms of service
    ...
    /// </returns>
    public virtual Task<CheckoutCompletedDto> PrepareCheckoutCompletedDtoAsync(Order order)
    {
        ArgumentNullException.ThrowIfNull(order);

        var model = new CheckoutCompletedDto
        {
            OrderId = order.Id,
    ...
    /// </returns>
    public virtual Task<CheckoutProgressDto> PrepareCheckoutProgressDtoAsync(CheckoutProgressStep step)
    {
        var model = new CheckoutProgressDto { CheckoutProgressStep = step };
    ...
    /// </returns>
    public virtual async Task<OnePageCheckoutDto> PrepareOnePageCheckoutDtoAsync(IList<ShoppingCartItem> cart)
    {
        ArgumentNullException.ThrowIfNull(cart);

        var customer = await _workContext.GetCurrentCustomerAsync();

        var model = new OnePageCheckoutDto
        {
            ...
            BillingAddress = await PrepareBillingAddressDtoAsync(cart, prePopulateNewAddressWithCustomerFields: true),
            DisplayCaptcha = await _customerService.IsGuestAsync(await _customerService.GetShoppingCartCustomerAsync(cart))
                             && _captchaSettings.Enabled && _captchaSettings.ShowOnCheckoutPageForGuests,
