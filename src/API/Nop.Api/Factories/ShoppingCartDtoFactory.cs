...
/// </summary>
public partial class ShoppingCartDtoFactory : IShoppingCartDtoFactory
{
    #region Fields
    ...
    public ShoppingCartDtoFactory(AddressSettings addressSettings,
    ...
    /// </returns>
    protected virtual async Task<IList<ShoppingCartDto.CheckoutAttributeDto>> PrepareCheckoutAttributeDtosAsync(
        IList<ShoppingCartItem> cart)
    {
        ArgumentNullException.ThrowIfNull(cart);

        var model = new List<ShoppingCartDto.CheckoutAttributeDto>();
    ...
    /// </returns>
    protected virtual async Task<ShoppingCartItemDto> PrepareShoppingCartItemDtoAsync(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
    {
        ArgumentNullException.ThrowIfNull(cart);
    ...
    /// </returns>
    protected virtual async Task<ShoppingCartDto.OrderReviewDataModel> PrepareOrderReviewDataModelAsync(IList<ShoppingCartItem> cart)
    {
        ArgumentNullException.ThrowIfNull(cart);

        var model = new ShoppingCartDto.OrderReviewDataModel
        {
            Display = true
        };
        ...
        {
            await _addressModelFactory.PrepareAddressDtoAsync(model.BillingAddress,
        ...
        if (await _shoppingCartService.ShoppingCartRequiresShippingAsync(cart))
        {
            ...
            if (!model.SelectedPickupInStore)
            {
                ...
                {
                    await _addressModelFactory.PrepareAddressDtoAsync(model.ShippingAddress,
                        address: address,
                        excludeProperties: false,
                        addressSettings: _addressSettings);
                }
            ...
            else
            {
                ...
                model.PickupAddress = new AddressDto
                {
    ...
    /// </returns>
    public virtual async Task<EstimateShippingDto> PrepareEstimateShippingDtoAsync(IList<ShoppingCartItem> cart, bool setEstimateShippingDefaultAddress = true)
    {
        ArgumentNullException.ThrowIfNull(cart);

        var model = new EstimateShippingDto
        {
    ...
    /// </returns>
    public virtual async Task<ShoppingCartDto> PrepareShoppingCartDtoAsync(ShoppingCartDto model,
        ...
        bool prepareAndDisplayOrderReviewData = false)
    {
        ...
        foreach (var couponCode in discountCouponCodes)
        {
            ...
            if (discount != null)
            {
                model.DiscountBox.AppliedDiscountsWithCodes.Add(new ShoppingCartDto.DiscountBoxModel.DiscountInfoModel
                {
                    Id = discount.Id,
        ...
        //checkout attributes
        model.CheckoutAttributes = await PrepareCheckoutAttributeDtosAsync(cart);

        //cart items
        foreach (var sci in cart)
        {
            var cartItemModel = await PrepareShoppingCartItemDtoAsync(cart, sci);
    ...
    /// </returns>
    public virtual async Task<MiniShoppingCartDto> PrepareMiniShoppingCartDtoAsync()
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var model = new MiniShoppingCartDto
        {
        ...
        if (customer.HasShoppingCartItems)
        {
            ...
            if (cart.Any())
            {
                             ...
                             .ToList())
                {
                    ...
                    var cartItemModel = new ShoppingCartItemOverviewDto
                    {
                        Id = sci.Id,
                        ProductId = sci.ProductId,
                        ProductName = await _localizationService.GetLocalizedAsync(product, x => x.Name),
    ...
    /// </returns>
    public virtual async Task<OrderTotalsDto> PrepareOrderTotalsDtoAsync(IList<ShoppingCartItem> cart, bool isEditable)
    {
        var model = new OrderTotalsDto
        {
        ...
        if (cart.Any())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        model.TaxRates.Add(new TaxRateDto
                        {
            ...
            if (appliedGiftCards != null && appliedGiftCards.Any())
            {
                ...
                {
                    var gcModel = new OrderTotalsDto.GiftCard
                    {
    ...
    /// </returns>
    public virtual async Task<EstimateShippingResultDto> PrepareEstimateShippingResultModelAsync(IList<ShoppingCartItem> cart, EstimateShippingDto request, bool cacheShippingOptions)
    {
        ArgumentNullException.ThrowIfNull(request);

        var model = new EstimateShippingResultDto();
        ...
        if (await _shoppingCartService.ShoppingCartRequiresShippingAsync(cart))
        {
            ...
            if (rawShippingOptions.Any())
            {
                foreach (var option in rawShippingOptions)
                {
                    ...
                    model.ShippingOptions.Add(new EstimateShippingResultDto.ShippingOptionDto
                    {
                        Name = option.Name,
    ...
    /// </returns>
    public virtual async Task<PictureDto> PrepareCartItemPictureDtoAsync(ShoppingCartItem sci, int pictureSize, bool showDefaultPicture, string productName)
    {
        ...
        }, NopDtoCacheDefaults.CartPictureDtoKey, sci, pictureSize, true, await _workContext.GetWorkingLanguageAsync(), _webHelper.IsCurrentConnectionSecured(), await _storeContext.GetCurrentStoreAsync());
