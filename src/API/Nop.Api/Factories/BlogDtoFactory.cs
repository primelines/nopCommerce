...
/// </summary>
public partial class BlogDtoFactory : IBlogDtoFactory
{
    #region Fields
    ...
    public BlogDtoFactory(BlogSettings blogSettings,
    ...
    public virtual async Task PrepareBlogPostDtoAsync(BlogPostDto model, BlogPost blogPost, bool prepareComments)
    {
    ...
    /// </returns>
    public virtual async Task<BlogPostListDto> PrepareBlogPostListDtoAsync(BlogPagingFilteringDto command)
    {
        ...
        var model = new BlogPostListDto
        {
            ...
            {
                var blogPostModel = new BlogPostDto();
                await PrepareBlogPostDtoAsync(blogPostModel, blogPost, false);
    ...
    /// </returns>
    public virtual async Task<BlogPostTagListDto> PrepareBlogPostTagListDtoAsync()
    {
        var model = new BlogPostTagListDto();
        var store = await _storeContext.GetCurrentStoreAsync();
        ...
        model.Tags.AddRange(tags.OrderBy(x => x.Name).Select(tag => new BlogPostTagDto
        {
            Name = tag.Name,
    ...
    /// </returns>
    public virtual async Task<List<BlogPostYearDto>> PrepareBlogPostYearModelAsync()
    {
        var store = await _storeContext.GetCurrentStoreAsync();
        var currentLanguage = await _workContext.GetWorkingLanguageAsync();
        var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopDtoCacheDefaults.BlogMonthsModelKey, currentLanguage, store);
        var cachedModel = await _staticCacheManager.GetAsync(cacheKey, async () =>
        {
            var model = new List<BlogPostYearDto>();
            ...
            if (blogPosts.Any())
            {
                ...
                foreach (var kvp in months)
                {
                    ...
                    {
                        var yearModel = new BlogPostYearDto
                        {
                            Year = date.Year
                        };
    ...
    /// </returns>
    public virtual async Task<BlogCommentDto> PrepareBlogPostCommentModelAsync(BlogComment blogComment)
    {
