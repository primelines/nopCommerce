...
using Nop.Api.Framework.Globalization;
...
/// </summary>
public partial class ApiWorkContext : IWorkContext
{
    #region Fields
    ...
    public virtual async Task SetCurrentCustomerAsync(Customer customer = null)
    {
        if (customer == null)
        {
            ...
            if (customer == null || customer.Deleted || !customer.Active || customer.RequireReLogin)
            {
                ...
                customer = await EngineContext.Current.Resolve<IAuthenticationService>().GetAuthenticatedCustomerAsync();
